...
export function StressIndicatorEnhanced({
  ...
  const showNotification =
    preferences?.stress_alerts_enabled &&
    latestStressLog?.stress_level > preferences?.stress_alert_threshold
      ...
      {showNotification && (
        <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive text-xs">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-bold mb-1">High Stress Alert!</p>
              <p>Your stress level is {latestStressLog?.stress_level}/10. Consider taking a break or using the buffer feature.</p>
            </div>
          </div>
        </div>
      )}
